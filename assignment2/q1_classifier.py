...
class SoftmaxModel(Model):
  ...
  def load_data(self):
    ...
    self.input_labels = np.ones((self.config.n_samples,), dtype=np.int32)#So here, we are making all the labels 1?
...
#    self.input_placeholder = tf.placeholder(tf.float64)#,shape=(Config.batch_size,Config.n_features))
#    self.labels_placeholder = tf.placeholder(tf.int64)#tf.float32, shape=(Config.batch_size,Config.n_classes))
    self.input_placeholder = tf.placeholder(tf.float64 ,shape=(Config.batch_size,Config.n_features))
    self.labels_placeholder = tf.placeholder(tf.float64, shape=(Config.batch_size,Config.n_classes))
    ### END YOUR CODE
  ...
  def add_training_op(self, loss):
    ...
    ### YOUR CODE HERE
    #raise NotImplementedError
    #Here loss would be like root of the tree and our optimizers would be able to do the trick.
    opt = tf.train.GradientDescentOptimizer(learning_rate=tf.constant(self.config.lr, tf.float64))
    ### END YOUR CODE
  ...
  def add_model(self, input_data):
    ...
    #raise NotImplementedError
    print input_data
    W = tf.Variable(np.zeros((self.config.n_features, self.config.n_classes)), dtype=np.float64)
    b = tf.Variable([0.0], dtype=tf.float64)  #If this does not work use: tf.matmul
    #So since W and b have been declared variable, the baove two lines would execute just once, but out would execute multiple times.
    out = softmax(tf.matmul(input_data, W) + b)#This way it will always return zero.
    ### END YOUR CODE
    return out
  ...
  def __init__(self, config):
    ...
    self.pred = self.add_model(self.input_placeholder) #So this input_placeholder is the key variable doing magic.
...
def test_SoftmaxModel():
  ...
  with tf.Graph().as_default():
    ...
    sess.run(init)             #What the hell is this step doing? #It is running the op, defined just a line above.

    losses = model.fit(sess, model.input_data, model.input_labels)    #Why r we passing model variables to model objects?
