...
def softmax(x):
  ...
  if len(x.shape) == 1:
        ...
        print x.shape
        m = tf.reshape(tf.reduce_max(x, axis = 1), (x.shape.as_list()[0],1))
        e = tf.exp(x-m)
        X = tf.reshape(tf.reduce_sum(e, axis = 1), (x.shape.as_list()[0],1))
        return e/X
  ### END YOUR CODE
...
def cross_entropy_loss(y, yhat):
  ...
  """
  ### YOUR CODE HERE
  #effY = tf.reduce_sum(tf.to_float(y) * yhat, axis = 1)
  print y.shape
  print yhat.shape
  effY = tf.reduce_sum(y * yhat, axis = 1)
  return -tf.reduce_sum(tf.log(effY))
