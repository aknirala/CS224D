...
class NERModel(LanguageModel):
  ...
  def load_data(self, debug=False):
    ...
    self.X_dev, self.y_dev = du.docs_to_windows(
        docs, word_to_num, tag_to_num, wsize=self.config.window_size)
    if debug:        #How to initialize this variable?
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      #raise NotImplementedError
      #First replace input with the wv from wv!!
      print "self.input_placeholder.shape"
      print self.input_placeholder.shape
      window = tf.nn.embedding_lookup(self.wv, self.input_placeholder)
      print "window Shape b4: "
      print window.shape
      #
      #Now, let's reshape it.
      window = tf.to_float(tf.reshape(window, (-1, self.config.window_size*self.config.embed_size)))
      print "window Shape a4: "
      print window.shape
      ### END YOUR CODE
      return window
  #
  def add_model(self, window):
    ...
    """
    ### YOUR CODE HERE
    #Objective is simple, window*W + b to hidden layer and so on
    xavier = xavier_weight_init()
    #print "Window shape: "
    #print window.shape
    #W =
    with tf.variable_scope("Layer"):
      W = tf.to_float(tf.Variable( xavier( (self.config.window_size*self.config.embed_size, self.config.hidden_size)  )   )  ) #Why give name?
      b1 = tf.to_float(tf.Variable(xavier((self.config.hidden_size, ))))
      h = tf.tanh(tf.matmul(window, W) + b1)
    with tf.variable_scope("Softmax"):
      U = tf.to_float(tf.Variable( xavier( (self.config.hidden_size, self.config.label_size)    )  ) )
      b2 = tf.to_float( tf.Variable( xavier( (self.config.label_size, ) ) ) )
      output = tf.matmul(h, U) +b2
      #It seems error/loss would be computed in just one go.
  ...
  def __init__(self, config):
    ...
    self.predictions = tf.nn.softmax(y)   #This is the predictions-----------------------------------------------------------------------------------------------------------------------
  ...
  def run_epoch(self, session, input_data, input_labels,
    ...
    dp = self.config.dropout           #It's value is 0.9, what the hell is this?
    # We're interested in keeping track of the loss and accuracy during training
    total_loss = []
    total_correct_examples = 0
    total_processed_examples = 0
    total_steps = len(orig_X) / self.config.batch_size #Shouldn't 1 be added to it, or roof should be taken
  ...
  def predict(self, session, X, y=None):
    ...
    if np.any(y):  #It would evaluate to false if all y are zero!!!!!
        ...
        #print type(data)
        #print data.shape
    for step, (x, y) in enumerate(data):  #Howz it working when np.any(y) is False
