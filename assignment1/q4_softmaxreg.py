...
#(lambda weights: softmaxRegression(dummy_features,
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    #Making matrix Y: one-hot vector representing output
    Y = np.zeros((N, C))
    Y[np.arange(N), labels] = 1
    grad = np.copy(weights)*regularization

    #raise NotImplementedError
    grad += np.dot(features.T, prob-Y)/N
